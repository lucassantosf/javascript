class CalcController{
	...
	calc(){			
		let last = '';
		this._lastOperatpr = this.getLastItem();
		if(this._operation.length < 3){
			let firstItem = this._operation[0];
			this._operation = [firstItem, this._lastOperatpr, this._lastNumber];
		}
		if(this._operation.length > 3){
		...
		console.log(this._lastOperatpr);
	...
	getLastItem(isOperator = true){
		...
		if(!lastItem){
			lastItem = (isOperator)? this._lastOperatpr : this._lastNumber;
		}		
		return lastItem;
	}
	...
	addOperation(value){
		...
		if(isNaN(this.getLastOperation())) {
			...
			if(this.isOperator(value)){
				...
				//situação inicial, para quando o vetor de operações esta vazio
